...
class Scanner:
    ...
    async def scan_once(self) -> list[dict]:
        ...
        results = await asyncio.gather(*(self._scan_symbol(s, by_symbol.get(s, {})) for s in symbols), return_exceptions=True)
        return [r for r in results if isinstance(r, dict)]

    async def _scan_symbol(self, symbol: str, ticker: dict) -> dict:
        k5, k15, k1h, premium, oi = await asyncio.gather(
        ...
        features = build_features(symbol, ticker, k5, k15, k1h, premium, oi)
    ...
    def _alert_text(self, f: dict, score: float, level: int, reason: str, plan: dict) -> str:
        return (
            ...
            f"RSI15m: {f['rsi_15m']:.1f} Funding: {f['funding_rate']:.4%}\n"
            f"原因: {reason}\n"
